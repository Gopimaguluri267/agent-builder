...
5. For the Sign-Off section (multi-approver, `DOC-STD-004`): `{{SIGNOFF_ROWS_HTML}}` is a table row per *required approver role* for this document type, not a single field set. For each role, render one `<tr>`: if that role hasn't signed yet, use `class="abd-signoff-row-pending"` with the name/signed-at cells showing "Pending"; if it has, use `class="abd-signoff-row-signed"` with the real name, role, and timestamp the calling skill supplied. Never pre-fill a name or timestamp before they're actually provided — a role with no captured sign-off is always rendered pending, never guessed. `{{EVIDENCE_HASH}}` is one value for the whole document (shared across every approver's row), not per-row. `status` (and `{{SIGNOFF_STATE_CLASS}}`) is `signed` only once every required role's row shows signed — if even one role is still pending, the document-level status stays `pending_signoff` even though some individual rows are signed.
